...
def test_extract_conflict_markers(fuzzy_rerere, fixture_path, tmp_path):
    # Copy fixture to temporary file
    test_file = tmp_path / "test.txt"
    shutil.copy(fixture_path / "simple_conflict.txt", test_file)
    
    conflicts = fuzzy_rerere._extract_conflict_markers(test_file)
...
def test_hash_conflict(fuzzy_rerere):
    conflict = "<<<<<<<\nversion A\n=======\nversion B\n>>>>>>>"
    hash1 = fuzzy_rerere._hash_conflict(conflict)
    hash2 = fuzzy_rerere._hash_conflict(conflict)
    
    assert isinstance(hash1, str)
    assert len(hash1) == 16  # Check hash length
    assert hash1 == hash2    # Check hash consistency

def test_calculate_context_similarity(fuzzy_rerere):
    context1_before = "def test():\n    x = 1\n"
    context1_after = "    return x\n"
    context2_before = "def test():\n    x = 1\n"
    context2_after = "    return x\n"
    
    similarity = fuzzy_rerere._calculate_context_similarity(
...
def test_mark_and_save_resolutions(fuzzy_rerere, fixture_path, tmp_path):
    # Setup test files
    ...
    pre_path = fuzzy_rerere.get_pre_path_from_file_path(conflict_file)
...
def test_hash_record(fuzzy_rerere):
    ...
    hash1 = fuzzy_rerere._hash_record(record1)
    hash2 = fuzzy_rerere._hash_record(record2)
